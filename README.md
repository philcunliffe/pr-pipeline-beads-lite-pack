# PR Pipeline for Beads-Lite

Author-side and review-side PR discipline distributed as a Gas City pack,
adapted for cities that use `gastown-beads-lite-pack` and SQLite
beads-lite stores.
...
**v0.1.0** — beads-lite conversion. Ships five formulas and four matching
wrapper commands:
...
| `mol-pr-triage`       | direct sling only                      | Gas City issue triage report; installed but not exposed as a wrapper |

The planning, blast-radius, review, and triage formulas are read-only for
repo files. `mol-pr-ship` may modify the diff during simplify and
review-iteration stages. Reports are written under the city
`.gc/pr-pipeline/<sub>/` path when available. None of the formulas push or
open PRs — those decisions stay with the caller.
...
```toml
[imports.pr-pipeline]
source = "packs/pr-pipeline-beads-lite-pack"

[defaults.rig.imports.pr-pipeline]
source = "packs/pr-pipeline-beads-lite-pack"
```

Then install the formulas into the city and rig beads-lite stores:

```sh
gc pr-pipeline install
```

The wrapper commands default to the `gastown-beads-lite` worker target:
...
conventions, writes a structured plan to the city `.gc/pr-pipeline` output
directory, and audits the plan against 19
...
Default agent for all wrappers is `gastown-beads-lite.polecat`. Override
with `--agent`:
...
```sh
gc sling api-server/gastown-beads-lite.polecat mol-pr-start --formula --var issue=1234
```
...
This pack does not provide agents or a beads provider. It expects the city
to import `gastown-beads-lite-pack`, configure:

```toml
[beads]
provider = "exec:/absolute/path/to/packs/gastown-beads-lite-pack/assets/scripts/gc-beads-lite.sh"
```

and expose rig workers such as:

```text
<rig>/gastown-beads-lite.polecat
```

Formula snippets may still show upstream `bd ...` examples. In a
beads-lite rig session, agents must translate those commands to:
