...
concurrency contexts, and writes a structured report to the city
`.gc/pr-pipeline/blast-radius/<key>.md` path when available. **No code
is written.**
...
Direct sling (skip this command):
  gc sling api-server/gastown-beads-lite.polecat mol-pr-blast-radius --formula \
      --var scope="FuncXYZ in pkg/foo" --var key=funcxyz

Output:
  Report at ${GC_CITY_PATH}/.gc/pr-pipeline/blast-radius/<key>.md when
