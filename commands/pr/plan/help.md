...
the city `.gc/pr-pipeline/plans/issue-<N>.md` path when available, and
audits the plan against 19 recurring review findings. **No code is written.**
...
Flags:
  ...
  --agent <name>      Agent name to sling to (default:
                      "gastown-beads-lite.polecat"). Set this if your
                      city's coding-worker pool uses a different name.
...
Examples:
  ...
  # Use a different configured worker:
...
Output:
  The plan is written to ${GC_CITY_PATH}/.gc/pr-pipeline/plans/issue-<N>.md
  when GC_CITY_PATH is available; otherwise <repo-root>/.gc/pr-pipeline.
