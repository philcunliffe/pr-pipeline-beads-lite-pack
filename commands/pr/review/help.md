...
pre-flags 7 recurring fixup themes, and writes a scorecard report to the
city `.gc/pr-pipeline/reviews/pr-<N>.md` path when available. **No fixes are applied** — apply
...
Direct sling (skip this command):
  gc sling api-server/gastown-beads-lite.polecat mol-pr-review --formula --var pr=1234
